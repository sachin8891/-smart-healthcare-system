...
Database Initialization & Seeder Script
Supports SQLite (zero-config embedded relational DB) and MySQL 8.0 connectivity.
...
SQLITE_DB_PATH = os.path.join(CURRENT_DIR, "healthcare.db")

def init_sqlite_database():
    """Initializes the SQLite database with normalized schema and comprehensive seed data."""
    print(f"[DB Setup] Initializing relational database at {SQLITE_DB_PATH}...")
    ...
    CREATE TABLE IF NOT EXISTS appointments (
        ...
        booking_lead_days INTEGER DEFAULT 3,
        booking_channel TEXT DEFAULT 'Online Portal',
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    CREATE TABLE IF NOT EXISTS medicine_reminders (
        ...
        reminder_type TEXT NOT NULL DEFAULT 'Dosage Alert',
    ...
    if cursor.fetchone()[0] == 0:
        print("[DB Setup] Seeding database with doctors, patients, appointments, prescriptions, and reminders...")

        # Doctors
        doctors = [
            (1, 'DOC-CARD-01', 'Dr. Arvind Sharma', 'Cardiologist', 'Cardiology', 15, 'Room 102', 800.0, 1),
            (2, 'DOC-NEUR-02', 'Dr. Radhika Kapoor', 'Neurologist', 'Neurology', 12, 'Room 205', 900.0, 1),
            (3, 'DOC-PEDI-03', 'Dr. Sameer Verma', 'Pediatrician', 'Pediatrics', 8, 'Room 108', 500.0, 1),
            (4, 'DOC-ORTH-04', 'Dr. Meena Swaminathan', 'Orthopedic Surgeon', 'Orthopedics', 18, 'Room 304', 750.0, 1),
            (5, 'DOC-GENM-05', 'Dr. Rajesh Deshmukh', 'General Physician', 'General Medicine', 10, 'Room 101', 400.0, 1),
            (6, 'DOC-DERM-06', 'Dr. Ananya Roy', 'Dermatologist', 'Dermatology', 7, 'Room 210', 600.0, 1)
        ]
        cursor.executemany("INSERT INTO doctors VALUES (?,?,?,?,?,?,?,?,datetime('now'),?)", doctors)
        ...
        cursor.executemany("INSERT INTO patients VALUES (?,?,?,?,?,?,?,?,?,?,datetime('now'))", patients)
        ...
        cursor.executemany("INSERT INTO appointments VALUES (?,?,?,?,?,?,?,?,?,?,?,?,datetime('now'))", appointments)
        ...
        cursor.executemany("INSERT INTO prescriptions VALUES (?,?,?,?,?,?,?,?,datetime('now'),?)", prescriptions)
        ...
        cursor.executemany("INSERT INTO prescription_medicines VALUES (?,?,?,?,?,?,?,?)", medicines)
        ...
        reminders = [
            ...
            (10, 11, 1, 'SMS', 'Dosage Alert', '09:00 AM', 'Hi Sachin, remember your morning muscle relaxant tablet (Thiocolchicoside) with food.', 'Pending', None),
            (11, 11, 1, 'SMS', 'Dosage Alert', '09:00 PM', 'Hi Sachin, take your evening muscle relaxant and apply heating pad before bed.', 'Pending', None),
            (12, 11, 1, 'Email', 'Follow-up', '11:00 AM', 'Hi Sachin Gurjar, your orthopedic follow-up with Dr. Meena Swaminathan is on 2026-09-29.', 'Pending', None)
        ]
        cursor.executemany("INSERT INTO medicine_reminders VALUES (?,?,?,?,?,?,?,?,datetime('now'),?)", reminders)

    conn.commit()
    conn.close()
    print("[DB Setup] Database initialization and seeding completed successfully.")
